...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'divicore-lib': 'https://github.com/99darwin/divicore-lib.git',
    'bitcore-node-divi': 'https://github.com/99darwin/bitcore-node-divi.git'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bitcore-node-divi.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bitcore-node-divi directory and configuration

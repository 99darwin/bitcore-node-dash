...
var BitcoinRPC = require('divid-rpc');
...
/**
 * Provides a friendly event driven API to divid in Node.js. Manages starting and
 * stopping divid as a child process for application support, as well as connecting
 * to multiple divid processes for server infrastructure. Results are cached in an
...
Bitcoin.prototype._initCaches = function() {
  ...
  this.utxosCache = LRU(0);
  this.txidsCache = LRU(0);
  this.balanceCache = LRU(0);
  this.summaryCache = LRU(0);
  this.blockOverviewCache = LRU(0);
  this.transactionDetailedCache = LRU(0);

  // caches valid indefinitely
  this.transactionCache = LRU(0);
  this.rawTransactionCache = LRU(0);
  this.blockCache = LRU(0);
  this.rawBlockCache = LRU(0);
  this.blockHeaderCache = LRU(0);
  this.zmqKnownTransactions = LRU(0);
  this.zmqKnownTransactionLocks = LRU(0);
  this.zmqKnownBlocks = LRU(0);
  this.lastTip = 0;
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getCurrentHeight', this, this.getCurrentHeight, 0]
  ];
  return methods;
};
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in divid config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in divid config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in divid config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './divi.conf');
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of bitcore-node-divi. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bitcore-node-divi. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bitcore-node-divi. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that divid is undergoing a reindex. ' +
             ...
             'of bitcore-node-divi services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/divi.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/divi.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Divi Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Bitcoin.prototype._refreshHeight = function() {
  var self = this;
  self.client.getBlockchainInfo(function(err, response) {
    if (!err && self.height < response.result.blocks) {
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Divi Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/divid.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned divi process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean divi process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting divi process');
log.info(JSON.stringify(options));
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Divi process unexpectedly exited with code:', code);
        log.warn('Restarting divi child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Divi process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn divid.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to divid.'));
    }
...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  function transformUnspentOutput(delta, utxo) {
  ...
  function updateWithMempool(confirmedUtxos, mempoolDeltas) {
    ...
    for (var i = 0; i < mempoolDeltas.length; i++) {
      ...
      var utxo = confirmedUtxos.filter(u => u.txid === delta.txid)[0] || {height: self.height};
      ...
      if (delta.prevtxid && delta.satoshis <= 0) {
        ...
        mempoolUnspentOutputs.push(transformUnspentOutput(delta, utxo));
      }
    }
...
Bitcoin.prototype.getCurrentHeight = function() {
  var self = this;
 ...
 * @param {Object} txid - A divi transaction id
 * @param {Function} callback
 */
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('divid spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('divid process did not exit'));
      }
